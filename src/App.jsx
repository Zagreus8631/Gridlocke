...
for (let i = 0; i < 9; i++) {
  ...
  const offsets = getOffsets(pokemonInTeam.pattern);

  const offset = offsets.find((_, idx) => {
    const pos = getGridIndex(baseIndex, offsets[idx]);
    return pos === index;
  });

  if (!offset) continue;

  const offsetIndex = offsets.findIndex(o => {
    const pos = getGridIndex(baseIndex, o);
    return pos === index;
  });

  const patternIndices = pokemonInTeam.pattern
    .map((v, idx) => (v ? idx : null))
    .filter(v => v !== null);

  const patternIndex = patternIndices[offsetIndex];

  if (patternIndex !== undefined) {
    newPattern[patternIndex] = false;
  }
